...
def perfil():
    gestion = GestionUsuarios()
    usuario = gestion.obtenerUsuarioPorId(session["usuario_id"])
...
def detalle_foto(foto_id):
    ...
    for c in resultado["comentarios"]:
        if c.autor_id not in autores:
            try:
                u = gestion.obtenerUsuarioPorId(c.autor_id)
